...
const Announcement = () => {
  ...
  // Fetch Batch Codes and Groups based on activeRole
  useEffect(() => {
    ...
    // Check which data to load based on the role selected
    if (activeRole === 'projectIncharge' || activeRole === 'hod' || activeRole === 'tutor') {
      fetchBatchCodes(); // Fetch batch codes for projectIncharge, HoD, and tutor
    } else if (activeRole === 'projectGuide') {
      fetchGroups(); // Fetch groups for projectGuide
    }
  }, [activeRole, token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:5000/api/faculty/postAnnouncement',
        announcementData,
        {
          headers: {
  ...
  return (
    <div className="announcement-form-container">
      ...
      <form onSubmit={handleSubmit} className="announcement-form">
        ...
        {(activeRole === 'tutor' || activeRole === 'projectIncharge' || activeRole === 'hod') && (
          <div className="form-group">
